    ...
    class mainModel
    {
        ...
        private string statusPldGet(BufferSpace.PLD pld)
        {
            ...
            if (pld.procesStart)
            {
                ...
                int min = Convert.ToInt32((pld.runTimesSet * pld.lengthSet - (pld.length + ((pld.runTimes) * pld.lengthSet))) * 60 / (pld.speed == 0 ? 0.00001 : pld.speed));
